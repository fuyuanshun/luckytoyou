...
function loadInit(){
	...
	httpRequest.open('GET', 'http://localhost:8080/luck/get', true);//第二步：打开连接 将请求参数写在url中  ps:"http://localhost:8080/rest/xxx"
	...
	httpRequest.onreadystatechange = function () {
		if (httpRequest.readyState == 4 && httpRequest.status == 200) {
			...
			data = eval(json);
			console.log(data);
			console.info("iiiii:"+data.length);
...
$("document").ready(function(){
    ...
    if(localStorage.getItem("pf")){
		...
		dynamicLoading.css("/resources/css/style"+pf+".css");
		$("#bodybg img").attr("src","/resources/images/bodybg"+pf+".jpg");
